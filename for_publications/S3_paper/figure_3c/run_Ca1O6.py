...
def run():
  ...
  d_min=1.93
  monomer='lm'
  b_iso_mn = {'lm':31.0, 'sm':32.0}
  b_iso_o = {'lm':30, 'sm':30}
  b_iso_o2 = {'lm':28, 'sm':29}
  mn = maptbx.atom_curves(scattering_type="Ca", scattering_table="wk1995")
  ...
  distances1 = {'lm':2.70, 'sm':2.60}
  dist1  = distances1[monomer]
  dist2  = 2.40
  q1 = 0.65
  q2 = 0.00
  ...
  with open('absolute_scale_csv/2F_%s_CA1O6_map_values_noNormalization.csv'%monomer, 'r') as fin:
    for line in fin:
      if line !='\n':
        bx = line.split(',')
        if 'atoms' in line:
  ...
  dump('data_%s_Ca1O6.pkl'%monomer, dumper)
  plt.legend()
  #plt.savefig('fig_Ca1O6.pdf', format='pdf')
  plt.show()
