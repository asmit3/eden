...
def run():
  ...
  d_min=1.93
  monomer='lm'
  b_iso_mn = {'lm':27.0, 'sm':28.0} 
  b_iso_o = {'lm':30, 'sm':30} 
  b_iso_o2 = {'lm':28, 'sm':28} # 28 worked for sm 
  ...
  im_mn  = mn.image(d_min=d_min, b_iso=b_iso_mn[monomer], radius_min=-5, radius_max=5)
  im_o   =  o.image(d_min=d_min, b_iso=b_iso_o[monomer], radius_min=-5, radius_max=5)
  im_o2  =  o.image(d_min=d_min, b_iso=b_iso_o2[monomer], radius_min=-5, radius_max=5)
  dist1  = 1.75
  if monomer=='lm'
    dist2  = 2.1
  if monomer=='sm'
  ...
  for r_mn, vals_mn,  r_o, vals_o,  r_o2, vals_o2  in zip(im_mn.radii,  im_mn.image_values,
    ...
    if i is not None:
      ...
      s00 = s00 + 0.00*im_o.image_values[i]
      s25 = s25 + 0.25*im_o.image_values[i]
      s50 = s50 + 0.50*im_o.image_values[i]
      s75 = s75 + 0.75*im_o.image_values[i]
      s100 = s100 + 1.00*im_o.image_values[i]

    i = closest_index(arr=im_o2.radii-dist2, value=r_mn)
    if i is not None:
      s = s + q2*im_o2.image_values[i]
      s00 = s00 + q2*im_o2.image_values[i]
      s25 = s25 + q2*im_o2.image_values[i]
      s50 = s50 + q2*im_o2.image_values[i]
      s75 = s75 + q2*im_o2.image_values[i]
      s100 = s100 + q2*im_o2.image_values[i]
  ...
  with open('absolute_scale_csv/2F_%s_MN1O6_map_values_noNormalization.csv'%(monomer), 'r') as fin:
    for line in fin:
      if line !='\n':
        bx = line.split(',')
        if 'atoms' in line:
        ...
        dist_values.append(float(bx[0])-0.03) # Subtracting here to center the plot
  ...
  ax.scatter(all_r_mn, all_s00, label = 'Simulation:Ox=0.00', c=mpl.colors.rgb2hex(cmap(0.3), keep_alpha=True), linewidth=1)
  ax.scatter(all_r_mn, all_s25, label = 'Simulation:Ox=0.25', c=mpl.colors.rgb2hex(cmap(0.5), keep_alpha=True), linewidth=1)
  ax.scatter(all_r_mn, all_s50, label = 'Simulation:Ox=0.50', c=mpl.colors.rgb2hex(cmap(0.7), keep_alpha=True), linewidth=1)
  ax.scatter(all_r_mn, all_s75, label = 'Simulation:Ox=0.75', c=mpl.colors.rgb2hex(cmap(0.85), keep_alpha=True), linewidth=1)
  ax.scatter(all_r_mn, all_s100, label= 'Simulation:Ox=1.00', c=mpl.colors.rgb2hex(cmap(1.0), keep_alpha=True), linewidth=1)
  ...
  # Real data
  ax.scatter(dist_values, map_values, label='2F XFEL data', c='green')
  ...
  dump('data_%s_Mn1O6.pkl'%monomer, dumper)
  plt.legend()
  #plt.xlabel('Distances along O-Mn-O')
  plt.tight_layout()
  plt.show()
  #plt.savefig('fig_Mn1O6.pdf')
