...
def create_mtz(mtz_file, output_dir, base_seed=None):
    ...
    try:
        ...
        get_input = input
    try:
        user_input = get_input("Enter a seed (or press Enter to use random seed): ").strip()
    ...
    if user_input == "":
        ...
        try:
            base_seed = int(user_input)
        except ValueError:
            print("Invalid input. Using random seed instead.")
            base_seed = np.random.randint(0, 2**32 - 1)
...
def main():
    ...
    default_dir = "/pscratch/sd/k/kkondaka/newfolder/endrapid_test_8_28_v2/tutorial_endrapid/tutorial_data"
    ...
    while True:
        try:
            ...
            get_input = input
        try:
            user_input = get_input("Enter a directory path (or press Enter to use default): ").strip()
        except (EOFError, KeyboardInterrupt):
...
def subprocess_main(directory_to_search):
    end_rapid_folders = get_endrapid_paths(directory_to_search)
    structure_files = find_structure_files(directory_to_search, ('.pdb', '.mtz', '.cif'))
