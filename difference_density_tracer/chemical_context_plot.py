...
class map_value_calculator(object):
  ...
  def get_map_values_around_residue(self):
    ...
    neg_stdev = np.std(neg_vals)
    pos_stdev = np.std(pos_vals)
    if True:
      indices=range(len(pos_vals))
      plt.fill_between(indices,list(np.array(pos_vals)-pos_stdev), list(np.array(pos_vals)+pos_stdev),color='g', alpha=0.3)
      plt.fill_between(indices,list(np.array(neg_vals)+neg_stdev), list(np.array(neg_vals)-neg_stdev),color='r', alpha=0.3)
      plt.plot(pos_vals,'-',color='g')
      plt.plot(neg_vals,'-',color='r')
    else:
      indices=range(len(pos_vals))
      plt.errorbar(indices, pos_vals, yerr=pos_stdev, fmt='-o',color='g')
      plt.errorbar(indices, neg_vals, yerr=neg_stdev, fmt='-o',color='r')
    # Demarcate in x
    ...
    plt.axhline(-2.5, color='grey', linestyle="--", alpha=0.3)
    plt.axhline(np.mean(pos_vals), color='pink', alpha=0.3)
    plt.axhline(np.mean(neg_vals), color='pink', alpha=0.3)
    #
    ...
    plt.ylabel('Summed Map value ($\sigma$ scaled)', fontsize=14)
    plt.xlabel('Residue/atom id', fontsize=14)
    
    if False:
      from libtbx.easy_pickle import dump
      dumper = (pos_vals, neg_vals, pos_stdev, neg_stdev, labels)
      dump(self.plot_save_fname+'.pickle', dumper)
